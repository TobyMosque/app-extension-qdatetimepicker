<template>
  <q-form ref="form" class="col col-12 col-md-6 col-lg-4 q-pa-md q-gutter-y-sm bg-grey-2" :class="{ 'bg-grey-2': !dark, 'bg-grey-9': dark }">
    <template v-if="mode !== 'date' || !this.format24h">
      ...
      <q-datetime-picker :mode="mode" :rules="rules" :color="color" :date-options="dateOptions" :time-options="timeOptions" :default-standard="standard" :format24h="format24h" :display-value="displayValue" :dark="dark" outlined clearable disable v-model="__value" :label="`${prefix} Readonly`"></q-datetime-picker>
      <q-datetime-picker :mode="mode" :rules="rules" :color="color" :date-options="dateOptions" :time-options="timeOptions" :default-standard="standard" :format24h="format24h" :display-value="displayValue" :dark="dark" outlined clearable disable-popup v-model="__value" :label="`${prefix} No Popup`"></q-datetime-picker>
      <q-datetime-picker :mode="mode" :rules="rules" :color="color" :date-options="dateOptions" :time-options="timeOptions" :default-standard="standard" :format24h="format24h" :display-value="displayValue" :dark="dark" outlined clearable readonly v-model="__value" :label="`${prefix} Disabled`"></q-datetime-picker>
    </template>
